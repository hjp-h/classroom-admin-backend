...
class FileService {
  ...
  // 将图片信息插入到apply_file表
  async saveMaintenanceImg(filename, mimetype, size, applyId, userId) {
    const statement = `
      INSERT INTO maintenance_file(filename,mimetype,size,apply_id,user_id) value(?,?,?,?,?)
    `
    try {
      const [result] = await connection.execute(statement, [filename, mimetype, size, applyId, userId])
      return result
    } catch (e) {
      console.log(e)
      return -1
